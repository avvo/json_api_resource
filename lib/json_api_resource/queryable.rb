  ...
  module Queryable
    ...
    module ClassMethods
      ...
      def find(id)
        ...
        results = results.size == 1 ? single_result(results) : results
      ...
      def create(attr = {})
        run_callbacks :create do
          package_result(new(:client => self.client_klass.create(attr)))
        end
      end

      def where(opts = {})
        opts[:per_page] = opts.fetch(:per_page, self.per_page)
        results = (self.client_klass.where(opts).all).map! do |result|
          self.new(:client => result)
        end
        package_result(results)
      rescue JsonApiClient::Errors::ServerError => e
      ...
      def package_result(result)
        result.meta.deep_symbolize_keys!
        result
      end
